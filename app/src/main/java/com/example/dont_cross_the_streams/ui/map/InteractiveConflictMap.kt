...
fun InteractiveConflictMap(
    ...
    Box(modifier = modifier.fillMaxSize()) {
        if (LocalInspectionMode.current) {
            ...
            AndroidView(
                ...
                update = { mapView ->
                    ...
                    // Only animate if position target is programmatically moved (e.g. preset jump or direction button)
        ...
        // Floating Navigation Helpers (Directional Pan Pad + Zoom In/Out + Recenter)
        Card(
            modifier = Modifier
            ...
            Column(
                ...
                IconButton(
                    onClick = { onPanDirection(1.0, 0.0) }, // North
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    IconButton(
                        onClick = { onPanDirection(0.0, -1.0) }, // West
                        modifier = Modifier.size(32.dp)
                    ) {
                        Icon(
                    ...
                    IconButton(
                        onClick = { onPanDirection(0.0, 1.0) }, // East
                        modifier = Modifier.size(32.dp)
                    ) {
                        Icon(
                ...
                IconButton(
                    onClick = { onPanDirection(-1.0, 0.0) }, // South
                    modifier = Modifier.size(32.dp)
                ) {
                    Icon(
...
private class ConflictMapOverlay(
    ...
    override fun draw(canvas: Canvas?, mapView: MapView?, shadow: Boolean) {
        ...
        // 1. Draw Population Density Zones
        populationZones.forEach { zone ->
        ...
        // 2. Draw Linear Infrastructure Barriers
        barriers.forEach { barrier ->
        ...
        // 3. Draw Collision & Roadkill Hotspot Pins
        collisionHotspots.forEach { hotspot ->
        ...
        // 4. Draw Wildlife Occurrence Pins
        wildlifeOccurrences.forEach { occ ->
...
@Composable
fun InteractiveConflictMapPreview() {
    DontcrossthestreamsTheme {
        InteractiveConflictMap(
            ...
            panOffsetX = 0f,
            panOffsetY = 0f,
            wildlifeOccurrences = emptyList(),
            collisionHotspots = emptyList(),
            barriers = emptyList(),
            populationZones = emptyList(),
